...
{
    public ImageResizerView() => InitializeComponent();
    private async void OnPickInput(object? s, RoutedEventArgs e)
    {
        if (DataContext is not ImageResizerViewModel vm) return;
        var top = TopLevel.GetTopLevel(this); if (top is null) return;
        var fs = await top.StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions { Title = "Pick image", AllowMultiple = false });
    ...
    private async void OnPickOutput(object? s, RoutedEventArgs e)
    {
        if (DataContext is not ImageResizerViewModel vm) return;
        var top = TopLevel.GetTopLevel(this); if (top is null) return;
        var f = await top.StorageProvider.SaveFilePickerAsync(new FilePickerSaveOptions { Title = "Save image" });
