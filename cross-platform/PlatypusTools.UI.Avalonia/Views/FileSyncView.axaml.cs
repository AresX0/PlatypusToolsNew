...
public partial class FileSyncView : UserControl
{
    public FileSyncView() => InitializeComponent();
    ...
    {
        if (DataContext is not FileSyncViewModel vm) return;
        var top = TopLevel.GetTopLevel(this); if (top is null) return;
        var folders = await top.StorageProvider.OpenFolderPickerAsync(new FolderPickerOpenOptions { Title = src ? "Source" : "Destination", AllowMultiple = false });
        var p = folders.Select(f => f.TryGetLocalPath()).FirstOrDefault(p => !string.IsNullOrEmpty(p));
        if (string.IsNullOrEmpty(p)) return;
        if (src) vm.Source = p!; else vm.Destination = p!;
    }
}
