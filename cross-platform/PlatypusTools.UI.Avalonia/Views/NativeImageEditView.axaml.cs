...
{
    public NativeImageEditView() => InitializeComponent();
    private async void OnPickInClicked(object? sender, RoutedEventArgs e)
    {
        if (DataContext is not NativeImageEditViewModel vm) return;
        var top = TopLevel.GetTopLevel(this); if (top is null) return;
        var fs = await top.StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions { Title = "Image in" });
    ...
    private async void OnPickOutClicked(object? sender, RoutedEventArgs e)
    {
        if (DataContext is not NativeImageEditViewModel vm) return;
        var top = TopLevel.GetTopLevel(this); if (top is null) return;
        var f = await top.StorageProvider.SaveFilePickerAsync(new FilePickerSaveOptions { Title = "Image out", DefaultExtension = "png" });
