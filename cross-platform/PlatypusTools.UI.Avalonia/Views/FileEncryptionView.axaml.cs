...
{
    public FileEncryptionView() => InitializeComponent();
    private async void OnPickInput(object? s, RoutedEventArgs e)
    {
        ...
        var fs = await top.StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions { Title = "Pick file", AllowMultiple = false });
    ...
    private async void OnPickOutput(object? s, RoutedEventArgs e)
    {
        if (DataContext is not FileEncryptionViewModel vm) return;
        var top = TopLevel.GetTopLevel(this); if (top is null) return;
        var f = await top.StorageProvider.SaveFilePickerAsync(new FilePickerSaveOptions { Title = "Save file" });
