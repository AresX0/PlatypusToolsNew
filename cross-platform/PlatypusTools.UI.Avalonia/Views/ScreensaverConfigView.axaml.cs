...
public partial class ScreensaverConfigView : UserControl
{
    public ScreensaverConfigView() => InitializeComponent();
    private async void OnPickClicked(object? sender, RoutedEventArgs e)
    {
        if (DataContext is not ScreensaverConfigViewModel vm) return;
        ...
        if (!string.IsNullOrEmpty(p)) vm.ImageFolder = p!;
    }
}
