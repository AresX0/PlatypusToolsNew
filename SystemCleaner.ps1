...
function Write-SecLog {
    ...
    $ts = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    $line = "[$ts] $Message"
    try { Add-Content -LiteralPath $SecLogPath -Value $line -Encoding UTF8 } catch {}
}
    ...
    <DockPanel LastChildFill="True">
        <Border DockPanel.Dock="Top" Background="{StaticResource HeaderBrush}" Padding="12" CornerRadius="6" Margin="8">
            <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
                ...
                <TextBlock Text="SystemCleaner" FontSize="16" FontWeight="Bold" Foreground="White"/>
            </StackPanel>
        </Border>
...
$appBasePath = if (Test-Path -LiteralPath $PlatypusAssets) { $PlatypusAssets } elseif ($PSScriptRoot) { $PSScriptRoot } elseif ($scriptPath) { Split-Path -Path $scriptPath -Parent } else { Split-Path -Path ([System.Diagnostics.Process]::GetCurrentProcess().MainModule.FileName) -Parent }
$iconCandidates = @(
    (Join-Path $PlatypusAssets 'platypus.png')
    (Join-Path $PlatypusAssets 'platypus.ico')
...
# --- Config init (safe) ---
$configRoot = Join-Path $PlatypusData "SystemCleaner"
...
# --- Populate UI ---
foreach ($d in $config.ExcludedDirs) { if ($d) { [void]$dirList.Items.Add($d) } }
