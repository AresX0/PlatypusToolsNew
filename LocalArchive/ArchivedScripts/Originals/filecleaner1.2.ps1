...
<Window xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation'
        ...
        Title='Batch File Renamer with Metadata' Height='980' Width='1220'
        WindowStartupLocation='CenterScreen' MinHeight='760' MinWidth='980'>
  <Grid Margin='10'>
    <Grid.RowDefinitions>
    ...
    <StackPanel Orientation='Horizontal' Grid.Row='0' Margin='0,0,0,8'>
      ...
      <TextBox Name='TxtFolder' Width='820' Margin='6,0,6,0'/>
      ...
      <Grid>
        ...
        <StackPanel Grid.Column='0' Orientation='Vertical'>
          ...
          <StackPanel Orientation='Horizontal' Margin='0,0,0,6'>
            ...
            <Label Content='Change Prefix (Old → New)' VerticalAlignment='Center' Margin='6,0,0,0'/>
          </StackPanel>
          <StackPanel Orientation='Horizontal' Margin='0,0,0,6'>
            <Label Content='Old Prefix' Width='80' VerticalAlignment='Center'/>
            <TextBox Name='TxtOldPrefix' Width='200' IsEnabled='False' Margin='6,0,12,0'/>
            <Label Content='New Prefix' Width='80' VerticalAlignment='Center'/>
            <TextBox Name='TxtNewPrefix' Width='200' IsEnabled='False' Margin='6,0,0,0'/>
            <CheckBox Name='ChkRequireExistingPrefix' Content='Only files with Old Prefix' Margin='12,0,0,0' VerticalAlignment='Center' IsEnabled='False'/>
          </StackPanel>

          <!-- Add prefix to all -->
          <StackPanel Orientation='Horizontal' Margin='0,0,0,6'>
            <CheckBox Name='ChkAddPrefixAll' Content='Add Prefix to All Files' VerticalAlignment='Center'/>
            <TextBlock Text='(prepend new prefix to every file)' VerticalAlignment='Center' Foreground='Gray' Margin='12,0,0,0'/>
          </StackPanel>

          <!-- Existing prefix only -->
          <StackPanel Orientation='Vertical' Margin='0,0,0,6'>
            <StackPanel Orientation='Horizontal'>
              <CheckBox Name='ChkCleanOnlyIfOldPrefix' Content='Only clean files matching Old Prefix' VerticalAlignment='Center'/>
              <TextBlock Text='(keeps prefix intact, only updates episode/cleaning)' VerticalAlignment='Center' Foreground='Gray' Margin='12,0,0,0'/>
            </StackPanel>
            <StackPanel Orientation='Horizontal' Margin='24,4,0,0'>
              <Label Content='Old Prefix' Width='80' VerticalAlignment='Center'/>
              <TextBox Name='TxtCleanOnlyPrefix' Width='200' Margin='6,0,0,0'/>
            </StackPanel>
          </StackPanel>
          ...
          <StackPanel Orientation='Horizontal' Margin='0,0,0,6'>
            ...
            <Label Content='Season' VerticalAlignment='Center' Margin='6,0,0,0'/>
            <TextBox Name='TxtSeason' Width='60' Text='1' Margin='6,0,0,0'/>
            <Label Content='Digits' VerticalAlignment='Center' Margin='8,0,0,0'/>
            ...
            <CheckBox Name='ChkSeasonBeforeEpisode' Content='Use S##E### format' Margin='12,0,0,0' VerticalAlignment='Center'/>
          </StackPanel>

          <StackPanel Orientation='Horizontal' Margin='0,0,0,6'>
            <CheckBox Name='ChkAddEpisode' VerticalAlignment='Center'/>
            <Label Content='Episode start' VerticalAlignment='Center' Margin='6,0,0,0'/>
            <TextBox Name='TxtStart' Width='60' Text='1' Margin='6,0,0,0'/>
            <Label Content='Digits' VerticalAlignment='Center' Margin='10,0,0,0'/>
        ...
        <!-- Right column: file types, metadata, actions -->
        <StackPanel Grid.Column='1' Orientation='Vertical' HorizontalAlignment='Right'>
          ...
          <CheckBox Name='ChkUseAudioMetadata' Content='Use audio metadata (artist/album/title)' Margin='0,0,6,0'/>
          <CheckBox Name='ChkUseVideoMetadata' Content='Use video metadata (show/title/season/episode)' Margin='0,0,6,0'/>
          ...
          <Button Name='BtnScan' Content='Scan / Preview' Width='180' Margin='0,6,0,6'/>
          <Button Name='BtnScanMeta' Content='Scan Metadata' Width='180' Margin='0,6,0,6'/>
          <Button Name='BtnApply' Content='Apply Changes' Width='180' Margin='0,6,0,6' IsEnabled='False'/>
          <Button Name='BtnUndo' Content='Undo Last' Width='180' Margin='0,6,0,6' IsEnabled='False'/>
          <Button Name='BtnExportCsv' Content='Export Preview CSV' Width='180' Margin='0,6,0,6' IsEnabled='False'/>
    ...
    <!-- Preview grid -->
    <DataGrid Grid.Row='2' Name='DgPreview' AutoGenerateColumns='False' CanUserAddRows='False'
      ...
      <DataGrid.Columns>
        ...
        <DataGridTextColumn Header='Status' Binding='{Binding Status}' IsReadOnly='True' Width='200'/>
    ...
    <TextBlock Grid.Row='4' Foreground='Gray'
               Text='Tip: Use Dry-run first. "Existing prefix only" uses its own Old Prefix box and keeps it intact while cleaning/renumbering.'
               Margin='0,8,0,0'/>
  </Grid>
</Window>
"@
...
$ChkCleanOnlyIfOldPrefix  = $window.FindName("ChkCleanOnlyIfOldPrefix")
$TxtCleanOnlyPrefix       = $window.FindName("TxtCleanOnlyPrefix")
...
$BtnBrowse.Add_Click({
    ...
    if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        $TxtStatusBar.Text = "Folder selected: $($dialog.SelectedPath)"
    }
})
...
# Sort key: portion of filename after any episode token
...
# Normalize filename (with optional cleaning)
...
function Generate-ProposedName {
    ...
    # Strip out any existing episode tokens (we’ll re‑add them)
    $base = $base -replace '(?i)(S\d{1,3}E\d{1,4}|E\d{1,4})[_\-\.\s]*', ''
    $base = $base.Trim()

    # Decide prefix handling
    $prefix = ''

    if ($existingPrefixOnly) {
        # Only operate on files that already have the specified prefix
        if ($file.BaseName -match "^$([regex]::Escape($cleanOnlyPrefix))([_\-\. ]?)") {
            $prefix = $cleanOnlyPrefix
        }
    }
    elseif ($addPrefixAll -and $newPrefix) {
        # Always add this prefix, even if file had none
        $prefix = $newPrefix
    }
    elseif ($newPrefix -and $oldPrefix -and $file.BaseName -match "^$([regex]::Escape($oldPrefix))([_\-\. ]?)") {
        # Replace old with new
        $prefix = $newPrefix
    }
    elseif ($newPrefix -and -not $oldPrefix) {
        # Apply new prefix if no old prefix defined
        $prefix = $newPrefix
    }
    elseif ($oldPrefix -and $file.BaseName -match "^$([regex]::Escape($oldPrefix))([_\-\. ]?)") {
        # Keep old prefix if present
        $prefix = $oldPrefix
    }

    # --- Ensure prefix is not duplicated ---
    if ($prefix) {
        $base = $base -replace "^(?i)$([regex]::Escape($prefix))([_\-\. ]?)", ''
        $base = $base.Trim(' ','_','-','.')
    }
...
$BtnScan.Add_Click({
    ...
    $cleanOnlyPrefix  = $TxtCleanOnlyPrefix.Text.Trim()
    $requireExistingPrefix = $ChkRequireExistingPrefix.IsChecked
    $existingPrefixOnly    = ($ChkCleanOnlyIfOldPrefix.IsChecked -and $cleanOnlyPrefix)
    $addPrefixAll     = $ChkAddPrefixAll.IsChecked
    ...
    $performClean     = $true
    if ($existingPrefixOnly -and -not $cleanOnlyPrefix) { $performClean = $false }
    ...
    # Sort alphabetically by post-episode portion if renumbering
    if ($renumberAll) {
    ...
    foreach ($file in $files) {
        $ext = $file.Extension
        $meta = @{}  # metadata reading can be added here if needed
        $episodeToken = if ($addEpisode) { Pad-Episode -num $index -pad $episodeDigits } else { $null }
        if ($addEpisode) { $index++ }

        $proposed = Generate-ProposedName -file $file -index $index -ext $ext `
            -oldPrefix $oldPrefix -newPrefix $newPrefix -cleanOnlyPrefix $cleanOnlyPrefix `
            ...
            -useAudioMeta $useAudioMeta -useVideoMeta $useVideoMeta -meta $meta `
            -requireExistingPrefix $requireExistingPrefix -existingPrefixOnly $existingPrefixOnly -addPrefixAll $addPrefixAll
    ...
    Set-Status "$($PreviewList.Count) files scanned."
})

# -----------------------
# Apply, Undo, Export, Reset
# -----------------------
...
$BtnApply.Add_Click({
    ...
    $BtnApply.IsEnabled = $false
    Set-Status "Rename complete. Log: $logFile"
})
