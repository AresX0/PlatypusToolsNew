...
$BtnDupScan.Add_Click({
  ...
  if ($LstDupScanLog) { $LstDupScanLog.Items.Add("--- Listing files ---") }
  ...
  $total = $files.Count
  if ($LstDupScanLog) {
    $LstDupScanLog.Items.Add("Found $total file(s)")
    foreach ($f in $files) { $LstDupScanLog.Items.Add($f.FullName) }
  }
  ...
  if ($jsonPath) {
    try {
      ...
      ($payload | ConvertTo-Json -Depth 6) | Out-File -FilePath $jsonPath -Encoding UTF8 -ErrorAction Stop
      if ($LstDupScanLog) { $LstDupScanLog.Items.Add("JSON updated (listed) -> $jsonPath") }
    }
    catch {
      if ($LstDupScanLog) { $LstDupScanLog.Items.Add("ERROR writing JSON (listed): $($_.Exception.Message)") }
    }
  }
  ...
  if ($LstDupScanLog) { $LstDupScanLog.Items.Add("--- Hashing (SHA-256) ---") }
  ...
  foreach ($f in $files) {
    ...
    try {
      ...
      if ($LstDupScanLog) { $LstDupScanLog.Items.Add("Hashed $($f.FullName) => $sha") }
    }
    catch {
      if ($LstDupScanLog) { $LstDupScanLog.Items.Add("ERROR hashing $($f.FullName): $($_.Exception.Message)") }
    }
  }

  $uniqueCount = ($hashEntries | Select-Object -ExpandProperty Sha256 -Unique).Count
  if ($LstDupScanLog) { $LstDupScanLog.Items.Add("Hashing complete. Files hashed=$($hashEntries.Count), Unique hashes=$uniqueCount") }

  # --- STEP 3: Save hashed JSON ---
  Set-DupStatus "Saving hash catalog..."
  if ($LstDupScanLog) { $LstDupScanLog.Items.Add("--- Saving JSON ---") }
  ...
  if ($jsonPath) {
    try {
      ...
      ($payload | ConvertTo-Json -Depth 6) | Out-File -FilePath $jsonPath -Encoding UTF8 -ErrorAction Stop
      if ($LstDupScanLog) { $LstDupScanLog.Items.Add("JSON updated (hashed) -> $jsonPath") }
    }
    catch {
      if ($LstDupScanLog) { $LstDupScanLog.Items.Add("ERROR writing JSON (hashed): $($_.Exception.Message)") }
    }
  }

  # --- STEP 4: Find duplicates ---
  Set-DupStatus "Finding duplicates..."
  if ($LstDupScanLog) { $LstDupScanLog.Items.Add("--- Finding duplicates ---") }
  ...
  if ($LstDupScanLog) { $LstDupScanLog.Items.Add("Duplicate rows prepared: $($dupes.Count)") }
  ...
  if ($LstDupScanLog) { $LstDupScanLog.Items.Add("Scan finished.") }
})
