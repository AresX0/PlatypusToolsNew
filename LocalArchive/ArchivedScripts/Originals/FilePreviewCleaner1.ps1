# RecentCleaner.ps1
# Patched: non-blocking status updates, removed routine MessageBox popups.
...
$required = @(
  ...
  "PreviewGrid","ShowPreview","ExportCSV","UndoLast","RunNow","SaveTask","ExitApp","StatusText"
)
$controls = @{}
    ...
    try {
        ...
        if ($durationMs -gt 0) {
            Start-Job -ScriptBlock {
                ...
                try { $w.Dispatcher.Invoke([action]{ $t.Text = "Ready" }) } catch {}
            } -ArgumentList $window, $statusText, $durationMs | Out-Null
        }
    } catch { }
}
