...
function Add-DupScanLogLine {
  ...
  $window.Dispatcher.Invoke([Action]{ $LstDupScanLog.Items.Add($Text) }) | Out-Null
}
...
$BtnDupScan.Add_Click({
  ...
  Set-DupProgress $true 0 0
  Set-DupStatus ("Scanning ({0})..." -f $method)
  Update-Status ("Scanning for duplicates ({0})..." -f $method)
  $dispatcher = $window.Dispatcher
  # Capture UI values before entering background task
  $extList = Get-DupExtensionsFromUI
  ...
  if ($jsonPath) {
    Write-DupLog ("JSON file created at {0}" -f $jsonPath) $logPath
  }
  else {
    Write-DupLog "JSON file could not be created; proceeding without JSON output." $logPath
    Update-Status "JSON file could not be created; continuing without JSON output."
  }
  Write-DupLog ("Scan start. Folder='{0}', Recurse={1}, Method={2}" -f $folder, $recurse, $method) $logPath
  # Capture local variables for background task
  $script:_scanFolder     = $folder
  $script:_scanRecurse    = $recurse
  $script:_scanMethod     = $method
  $script:_scanLogPath    = $logPath
  $script:_scanJsonPath   = $jsonPath
  $script:_scanExtensions = $extList
  $null = [System.Threading.Tasks.Task]::Run([Action]{
    $folder     = $script:_scanFolder
    $recurse    = $script:_scanRecurse
    $method     = $script:_scanMethod
    $logPath    = $script:_scanLogPath
    $jsonPath   = $script:_scanJsonPath
    $extensions = $script:_scanExtensions
    try {
      Set-DupStatus "Listing files..."; Write-DupLog "--- Listing files ---" $logPath
      $files = Get-DupFilteredFiles -Path $folder -Recurse:$recurse -Extensions $extensions
      $listedEntries = @()
      if ($files) {
        $listedEntries = $files | ForEach-Object { [PSCustomObject]@{ Path=$_.FullName; Size=$_.Length; LastWriteTime=$_.LastWriteTimeUtc; Sha256=$null } }
      }
      if ($jsonPath) { Write-DupJsonSnapshot -JsonPath $jsonPath -Folder $folder -Recurse $recurse -Method $method -Entries $listedEntries -Stage "listed" -LogPath $logPath }
      $total = if ($files) { $files.Count } else { 0 }
      $dispatcher.Invoke([Action]{
        if ($LstDupScanLog) {
          $LstDupScanLog.Items.Clear()
          if ($files) { foreach ($f in $files) { $LstDupScanLog.Items.Add($f.FullName) } }
          if ($LstDupScanLog.Items.Count -gt 0) { $LstDupScanLog.ScrollIntoView($LstDupScanLog.Items[$LstDupScanLog.Items.Count-1]) }
        }
      })
      Write-DupLog ("File list complete. Total={0}" -f $total) $logPath
      if (-not $files -or $files.Count -eq 0) {
        $dispatcher.Invoke([Action]{ $DgDupes.ItemsSource = $null; Set-DupProgress $false; Set-DupStatus "No files found."; Update-Status "No files found for duplicate scan."; $script:DupScanRunning = $false })
        return
      }
      Set-DupStatus "Hashing files..."; Write-DupLog "--- Hashing (SHA-256) ---" $logPath
      $idx = 0
      $hashEntries = @()
      foreach ($f in $files) {
        $idx++
        try {
          $sha = Get-FastFileHash -Path $f.FullName
          if (-not $sha) { Write-DupLog ("ERROR hashing {0}: hash not generated" -f $f.FullName) $logPath; continue }
          ...
          Write-DupLog ("Hashed {0} => {1}" -f $f.FullName, $sha) $logPath
        } catch {
          Write-DupLog ("ERROR hashing {0}: {1}" -f $f.FullName, $_.Exception.Message) $logPath
        }
        if (($idx % 25) -eq 0) {
          $current = $idx; $tot = $total
          $dispatcher.Invoke([Action]{ Set-DupProgress $true $current $tot; Set-DupStatus ("Hashing {0}/{1}" -f $current,$tot) })
        }
      }
      if ($LstDupScanLog) { $dispatcher.Invoke([Action]{ if ($LstDupScanLog.Items.Count -gt 0) { $LstDupScanLog.ScrollIntoView($LstDupScanLog.Items[$LstDupScanLog.Items.Count-1]) } }) }
      $uniqueCount = ($hashEntries | Select-Object -ExpandProperty Sha256 -Unique).Count
      Write-DupLog ("Hashing complete. Files hashed={0}, Unique hashes={1}" -f $hashEntries.Count, $uniqueCount) $logPath
      Set-DupStatus "Saving hash catalog..."; Write-DupLog "--- Saving JSON ---" $logPath
      if ($jsonPath) { Write-DupJsonSnapshot -JsonPath $jsonPath -Folder $folder -Recurse $recurse -Method $method -Entries $hashEntries -Stage "hashed" -LogPath $logPath }
      Set-DupStatus "Finding duplicates..."; Write-DupLog "--- Finding duplicates ---" $logPath
      $dupes = @()
      foreach ($grp in ($hashEntries | Group-Object Sha256)) {
      ...
      Write-DupLog ("Duplicate rows prepared: {0}" -f $dupes.Count) $logPath
      $dispatcher.Invoke([Action]{
        $DgDupes.ItemsSource = $dupes
        Set-DupProgress $false
        $msg = if ($dupes.Count -gt 0) { "Duplicate groups: {0} entries" -f $dupes.Count } else { "No duplicates found." }
        if ($jsonPath) { $msg = $msg + (" JSON saved: {0}" -f $jsonPath) }
        Set-DupStatus $msg
        Update-Status $msg
        $script:DupScanRunning = $false
      })
      ...
      $err = $_
      Write-DupLog ("FATAL error: {0}" -f $err.Exception.Message) $logPath
      $dispatcher.Invoke([Action]{
        Set-DupProgress $false
        Set-DupStatus "Scan failed; see console"
        Update-Status ("Duplicate scan failed: {0}" -f $err.Exception.Message)
        $script:DupScanRunning = $false
      })
    }
    finally {
      if ($script:DupScanRunning) { $dispatcher.Invoke([Action]{ Set-DupProgress $false; $script:DupScanRunning = $false }) }
      Write-DupLog "Scan finished." $logPath
    }
  })
})
